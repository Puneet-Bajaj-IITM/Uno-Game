function CCard(iX, iY, oParentContainer, szFotogram, iRank, iSuit, iUniqueID, iRot) {
    ...
    this._init = function (iX, iY, oParentContainer, szFotogram, iRank, iSuit, iUniqueID, iRot) {
        ...
        var oData = {   // image to use
            ...
            animations: {
                card_0_0: [0], card_0_1: [1], card_0_2: [2], card_0_3: [3], card_0_4: [4], card_0_5: [5], card_0_6: [6], card_0_7: [7],
    ...
    this._mouseDown = function (event) {
        ...
        // console.log(_oParent)
        
        var iThisTurn = s_oGame._oTurnManager.getTurn();
        if (iThisTurn === 0) {
            var iParent = s_oGame._aPlayersHand[iThisTurn].searchIndexCard(_oParent);
            // const socket = io()
            window.socket.emit("movePlayed", {
                roomID: ROOM_ID,
        ...
        // socket.on("movePlayedBy", async (data) => {
        //     console.log('1')
        //   console.log(data)
        //     if(data.cardMoveBy == PLAYER_ID){
        //         var oPlayerHand = s_oGame.getPlayersHand(PLAYER_ID);
        //         console.log(oPlayerHand)
        //         // s_oGame.cpuPlayCard(oPlayerHand.getCardByIndex(data.turn));
        //         s_oGame.playCard(oPlayerHand.getCardByIndex(data.turn), event)
        //     }
        // });
