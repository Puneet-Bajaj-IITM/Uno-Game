function CGameSingle(oData) {
    ...
    window.socket.on("movePlayedBy", async (data) => {
        ...
        var findIndex = 0;
        GAME_INFO.map((item, index) => {
            if (item.position == data.cardMoveBy) {
                findIndex = index;
            }
        })
        var oPlayerHand = s_oGame.getPlayersHand(findIndex);
        if (data.type == 'draw') {
            this.drawCards(findIndex, 1, 0, DRAW_TYPE_NORMAL);
        }
        else {
            if (data.cardMoveBy !== GAME_INFO[0].position) {
                var iThisTurn = this._oTurnManager.getTurn();
                var bCanPlay = false;

                bCanPlay = this.cardCanBePlayed(oPlayerHand.getCardByIndex(data.turn), iThisTurn);
                if (bCanPlay) {
                    this.cpuPlayCard(oPlayerHand.getCardByIndex(data.turn));
                }

            }
            else {
                this.playCard(oPlayerHand.getCardByIndex(data.turn));
            }
        }

    });

    window.socket.on("colorSelectedBy", async (data) => {
        this._onActionSelectColor(data.oData);
    });

    window.socket.on("drawFourSelectedBy", async (data) => {
        const oData = data.oData
        oData.playerindex = this._iNextPlayer
...
CGameSingle.prototype._setPieces = async function () {
    this._oDeck.initializeDeck();
    await this._oDeck.shuffle();
...
CGameSingle.prototype.onNextTurn = function () {
    ...
    if (iWinner !== null) {
        ...
        if (IS_HOST) {
            window.parent.postMessage({ type: 'finished_uno' }, '*');
            setTimeout(() => {
                window.socket.emit("gamefinished", {
                    roomID: ROOM_ID,
                    winner: GAME_INFO[iWinner]
                })
            }, 5000)
...
CGameSingle.prototype._applySelectColor = function () {
    if (this._iCurPlayer === 0) {
        ...
        oSelectColorPanel.addEventListener(ON_COLOR_SELECTED, function (iColor) {
            ...
            window.socket.emit("colorSelected", {
                roomID: ROOM_ID,
                cardMoveBy: GAME_INFO[0].position,
                oData: oData
            })
...
CGameSingle.prototype._applyDrawFourEffect = function () {
    if (this._iCurPlayer === 0) {
        ...
        oSelectColorPanel.addEventListener(ON_COLOR_SELECTED, function (iColor) {
            var oData = { colorindex: iColor };
            window.socket.emit("drawFourSelected", {
                roomID: ROOM_ID,
                cardMoveBy: GAME_INFO[0].position,
                oData: oData
            })
